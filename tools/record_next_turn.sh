...
#   3. Copies SAVE9.GAM into the MSVC-debug (ReMoM) build directory.
#   4. Launches ReMoM with --continue, the pre-Next-Turn scenario script,
...
#   5. After ReMoM exits, dumps SAVE1.GAM to SAVE1.txt.
...
REMOM_DIR="${REPO_ROOT}/out/build/MSVC-debug/bin/Debug"
...
echo "=== Step 0/5: Rebuild MSVC-headless-debug (HeMoM) and MSVC-debug (ReMoM) ==="
...
cmake --build "${REPO_ROOT}/out/build/MSVC-debug"          --config Debug --target ReMoM  >/dev/null
cmake --build "${REPO_ROOT}/out/build/MSVC-debug"          --config Debug --target savedump  >/dev/null
echo "  rebuilt: HeMoM (MSVC-headless-debug), ReMoM + savedump (MSVC-debug)"
...
echo
echo "=== Step 2/5: Copy SAVE9.GAM and baseline dump to ReMoM build ==="
if [ ! -x "${REMOM_DIR}/ReMoM.exe" ]; then
    echo "FAIL: ReMoM.exe not found at ${REMOM_DIR}"
    echo "      Build the MSVC-debug preset first."
    exit 1
fi
if [ ! -x "${REMOM_DIR}/savedump.exe" ]; then
    echo "FAIL: savedump.exe not found at ${REMOM_DIR}"
    echo "      Build the savedump target under MSVC-debug."
    exit 1
fi
cp "${HEMOM_DIR}/SAVE9.GAM" "${REMOM_DIR}/SAVE9.GAM"
cd "${REMOM_DIR}"
./savedump.exe SAVE9.GAM SAVE9.txt > /dev/null
echo "  copied:  ${REMOM_DIR}/SAVE9.GAM"
echo "  dumped:  ${REMOM_DIR}/SAVE9.txt"

echo
echo "=== Step 3/5: Launching ReMoM for manual Next Turn session ==="
echo
...
echo
read -p "Press Enter to launch ReMoM... "

rm -f next_turn.RMR
./ReMoM.exe --continue --scenario "${PRE_SCENARIO}" --record next_turn.RMR
...
echo "  dumped:  ${REMOM_DIR}/SAVE1.txt"
...
if [ ${DIFF_RC} -eq 0 ]; then
    ...
    echo "  diff written: ${REMOM_DIR}/next_turn_diff.txt (${DIFF_LINES} lines)"
else
...
echo
echo "Done.  Output files in: ${REMOM_DIR}"
