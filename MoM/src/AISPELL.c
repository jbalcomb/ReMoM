...
int16_t AITP_Unit_Enchantment(int16_t spell_target_type, int16_t * unit_idx, int16_t spell_idx, int16_t player_idx)
{
    ...
    if(spell_target_type != stt_Friendly_Unit)
    {
        Cast_Spell_Target_Error(spell_idx);
...
int16_t Select_Unit_For_Enchantment(int16_t player_idx, int32_t unit_enchantment, int16_t * targeted_unit_idx, int16_t flag1, int16_t flag2, int16_t flag3, int16_t flag4, int16_t flag5)
{
    ...
    for(itr_stacks = 0; itr_stacks < _ai_all_own_stack_count; itr_stacks++)
    {
        ...
        if(found_city_at_stack == ST_FALSE)
        {
            if(_ai_all_own_stacks[itr_stacks].value > max_score)
            {
                ...
                max_score = _ai_all_own_stacks[itr_stacks].value;
            }
        }
    }
    ...
    if(best_stack_idx == ST_UNDEFINED)
    {
        ...
        {
            if(
                _ai_all_own_stacks[itr_stacks].wx == _FORTRESSES[player_idx].wx
                &&
                _ai_all_own_stacks[itr_stacks].wy == _FORTRESSES[player_idx].wy
                &&
                _ai_all_own_stacks[itr_stacks].wp == _FORTRESSES[player_idx].wp
            )
            {
                best_stack_idx = itr_stacks;
            }
    ...
    for(itr_units = 0; itr_units < _units; itr_units++)
    {
        ...
        /* 
         * We initialize the unit score to its upkeep value.
         */
        ...
        if(flag4 == ST_TRUE)
        {
            ...
            if(
                (_unit_type_table[_UNITS[itr_units].type].Move_Flags & MV_FLYING)
                ||
                (_unit_type_table[_UNITS[itr_units].type].Move_Flags & MV_SWIMMING)
            )
            {
        ...
        if(current_unit_score > max_score)
        {
            best_unit_idx = itr_units;
            max_score = current_unit_score;
        }
    }
    ...
    {
        *targeted_unit_idx = best_unit_idx;
        return ST_TRUE;
    }
...
NOT IN USE
¿ ~== AITP_Attack_Terrain() ?
highest-value **contacted, non-neutral, non-own** enemy city
vs.
contacted enemy (non-neutral) city of highest value

**land, not volcano, not corrupted, not hills, not mountain, not node**
Square_Is_Land(), Square_Is_Volcano(), Square_Has_Corruption()
    Square_Is_Hills(), Square_Is_Mountain(), Square_Is_Node()
vs.
Square_Is_Land(), Square_Is_Volcano(), Square_Has_Corruption()
if(spell_idx != spl_Corruption)
    Square_Is_Mountain(), Square_Is_Hills(), Square_Is_River(), Square_Is_Node()

*/
int16_t AITP_Corruption(int16_t player_idx, int16_t * targeted_wx, int16_t * targeted_wy, int16_t * targeted_wp)
{
    int16_t is_city_area_corner_square = 0;
    int16_t city_wp = 0;
    int16_t target_wp = 0;
    int16_t city_area_wy = 0;
    int16_t target_wy = 0;
    int16_t target_wx = 0;
    int16_t square_ctr = 0;
    int16_t found_target = 0;
    int16_t wy_offset = 0;
    int16_t wx_offset = 0;
    int16_t best_city_idx = 0;
    int16_t best_city_value = 0;
    int16_t itr_cities = 0;
    int16_t city_area_wx = 0;
    ...
    for(itr_cities = 0; itr_cities < _cities; itr_cities++)
    {
        ...
        if(Test_Bit_Field(player_idx, (uint8_t *)&_CITIES[itr_cities].contacts) == 0)
        {
            continue;
        }

        if(_ai_all_enemy_city_values[itr_cities] <= best_city_value)
        {
            continue;
        }
        ...
        while(square_ctr < 25 && found_target == 0)
        {
            wx_offset = Random(5) - 3;
            wy_offset = Random(5) - 3;

            /* Check if it is a corner: (-2, -2), (-2, 2), (2, -2), (2, 2) */
            if(
                (wx_offset == -2 && wy_offset == -2)
                ||
                (wx_offset == -2 && wy_offset == 2)
            ...
            {
                is_city_area_corner_square = ST_TRUE;
            }
            else
            {
                is_city_area_corner_square = ST_FALSE;
            }

            city_area_wx = _CITIES[itr_cities].wx + wx_offset;
            city_area_wy = _CITIES[itr_cities].wy + wy_offset;

            if(is_city_area_corner_square != ST_FALSE)
            {
                square_ctr++;
                continue;
            }

            if(city_area_wx <= 0 || city_area_wx >= WORLD_WIDTH)
            {
                square_ctr++;
                continue;
            }
            ...
            best_city_idx = itr_cities;
            best_city_value = _ai_all_enemy_city_values[itr_cities];
            target_wx = city_area_wx;
            target_wy = city_area_wy;
            target_wp = city_wp;
            found_target = ST_TRUE;
