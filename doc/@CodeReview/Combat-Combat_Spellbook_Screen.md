...
- **The `j_` prefixes are overlay thunks**, not different functions. `Combat_Compose_Background` lives in `ovr163` ([Combat.c:22131](../../MoM/src/Combat.c#L22131)) and `Combat_Screen_Draw` in `ovr099`; both are outside `ovr112`, so the call goes through the overlay stub.
