...
| --- | --- | --- | --- |
| `Battle_Unit_Pict_Open` | [Combat.c:11960](../../MoM/src/Combat.c#L11960) | `ovr113/Battle_Unit_Pict_Open.asm` | 72 |
| `Summon_Demon` | [Combat.c:11675](../../MoM/src/Combat.c#L11675) | [ovr113/BU_SummonDemon__SEGRAX.asm](../../../STU-Extras/Piethawn/Piethawn/out/WIZARDS/ovr113/BU_SummonDemon__SEGRAX.asm) (172 lines) | faithful |
| `Prepare_Battle_Unit_Summons` | [Combat.c:4747](../../MoM/src/Combat.c#L4747) | [ovr098/UNIT_SummonToBattle__SEGRAX.asm](../../../STU-Extras/Piethawn/Piethawn/out/WIZARDS/ovr098/UNIT_SummonToBattle__SEGRAX.asm) (70 lines) | faithful |
| `Prepare_Battle_Unit` | [Combat.c:4774](../../MoM/src/Combat.c#L4774) | [ovr098/BU_UnitLoadToBattle__SEGRAX.asm](../../../STU-Extras/Piethawn/Piethawn/out/WIZARDS/ovr098/BU_UnitLoadToBattle__SEGRAX.asm) (121 lines) | faithful |
| `Battle_Unit_Summon_Animation` | [Spells133.c:889](../../MoM/src/Spells133.c#L889) | [ovr133/BU_CombatSummon__SEGRAX.asm](../../../STU-Extras/Piethawn/Piethawn/out/WIZARDS/ovr133/BU_CombatSummon__SEGRAX.asm) (229 lines) | faithful (R1 fixed) |
...
`o113p15`, walked 2026-08-27, 72 asm lines. Faithful, with a genuine OG defect that production
documents and works around in place.

## `Battle_Unit_Pict_Open` ([Combat.c:11960](../../MoM/src/Combat.c#L11960), asm 72)

Finds a free picture-buffer slot: mark every slot referenced by a live battle unit, then return the
first unmarked index.

**One stack local and its size is provable.** `battle_unit_slots` sits at `bp-24h` with `sub sp, 24h`
— 36 bytes, i.e. 18 `int16_t` entries — and `MAX_BATTLE_UNIT_COUNT` is 18
([MOM_DEF.h:65](../../MoX/src/MOM_DEF.h#L65)). `battle_unit_slot_idx` is CX.

Three loops, all faithful: clear to `ST_FALSE` (asm:8-19), then mark `battle_unit_slots[bufpi]` for
units with `bufpi > ST_UNDEFINED` **and** `status == bus_Active` (asm:20-51), then scan for the first
`ST_FALSE` and return its index (asm:52-67).

### The OGBUG is real — the function can fall off the end without setting a return value

asm:66-68:

```
cmp     _CX_battle_unit_slot_idx, e_MAX_BATTLE_UNIT_COUNT
jl      short loc_920B4
@@Done:
mov     sp, bp
pop     bp
...
When every slot is taken the third loop simply ends and control reaches `@@Done` with **no `mov ax,
...` on that path** — AX holds whatever the loop left in it. Production records this at
[Combat.c:11992](../../MoM/src/Combat.c#L11992) and adds `/* HACK */ return ST_UNDEFINED; // DNE in
Dasm`. That is the right call: an undefined return cannot be expressed in C, the addition is marked,
and the note points at `Hero_Slot_Open()` for the same shape done correctly elsewhere.
