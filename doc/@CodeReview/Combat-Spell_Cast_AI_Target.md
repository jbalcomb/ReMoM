...
**Status: DONE-DONE (2026-08-27).** Fourteen slots across `ovr091`, `ovr111` and `ovr112`, **3,892 asm lines - the whole cohort walked end to end, and all thirteen are faithful.** Five findings were raised and all five are fixed. Every slot in this cohort was homeless before this doc.

## Scope

| function | production | listing | asm | state |
| --- | --- | --- | --- | --- |
| `Target_Is_Visible` | [Combat.c:2958](../../MoM/src/Combat.c#L2958) | `ovr091/Target_Is_Visible.asm` | 121 | **full body walk — faithful** |
| `AITP_Combat_Spell` | [Combat.c:8751](../../MoM/src/Combat.c#L8751) | `ovr111/AITP_Combat_Spell.asm` | 1,665 | **full body walk — faithful** |
| `AITP_DispelMagic` | [Combat.c:9353](../../MoM/src/Combat.c#L9353) | `ovr111/AITP_DispelMagic.asm` | 445 | **full body walk — faithful** |
| `AITP_EarthToMud` | [CMBMAGIC.c:1512](../../MoM/src/CMBMAGIC.c#L1512) | `ovr112/AITP_EarthToMud.asm` | 267 | **full body walk — faithful** |
| `AITP_CracksCall` | [CMBMAGIC.c:1666](../../MoM/src/CMBMAGIC.c#L1666) | `ovr112/AITP_CracksCall.asm` | 239 | **full body walk — faithful** |
| `AITP_WarpCreature` | [Combat.c:9256](../../MoM/src/Combat.c#L9256) | `ovr111/AITP_WarpCreature.asm` | 210 | **full body walk — faithful** |
| `AITP_RecallHero` | [CMBMAGIC.c:1810](../../MoM/src/CMBMAGIC.c#L1810) | `ovr112/AITP_RecallHero.asm` | 162 | **full body walk — faithful** |
| `AITP_WarpWood` | [Combat.c:9216](../../MoM/src/Combat.c#L9216) | `ovr111/AITP_WarpWood.asm` | 161 | **full body walk — faithful** |
| `AITP_Healing` | [Combat.c:9181](../../MoM/src/Combat.c#L9181) | `ovr111/AITP_Healing.asm` | 128 | **full body walk — faithful** |
| `AITP_HolyWord` | [Combat.c:10115](../../MoM/src/Combat.c#L10115) | `ovr111/AITP_HolyWord.asm` | 76 | **full body walk — faithful** |
| `AITP_Disrupt` | [CMBMAGIC.c:1619](../../MoM/src/CMBMAGIC.c#L1619) | `ovr112/AITP_Disrupt.asm` | 66 | **full body walk — faithful** |
| `AITP_DarknessLight` | [Combat.c:9147](../../MoM/src/Combat.c#L9147) | `ovr111/AITP_DarknessLight.asm` | 54 | **full body walk — faithful** |
| `AITP_Disintegrate` | [Combat.c:9297](../../MoM/src/Combat.c#L9297) | `ovr111/UU15_AITP_Disintegrate.asm` | 172 | **full body walk — faithful** |
| `AITP_Word_Of_Recall` | [CMBMAGIC.c:1767](../../MoM/src/CMBMAGIC.c#L1767) | `ovr112/UU_AITP_WordofRecall.asm` | 126 | **full body walk — faithful** |
...
### `AITP_Combat_Spell` ([Combat.c:8751](../../MoM/src/Combat.c#L8751), asm 1,665)
...
### `AITP_Disintegrate` ([Combat.c:9297](../../MoM/src/Combat.c#L9297), asm 172)
...
### `AITP_Word_Of_Recall` ([CMBMAGIC.c:1767](../../MoM/src/CMBMAGIC.c#L1767), asm 126)
...
`s91p08`, walked 2026-08-27, 121 asm lines. The visibility gate every AI targeting slot in this
review calls before considering a unit — nine call sites in `Combat.c`, all of the form
`if(!Target_Is_Visible(battle_unit_idx)) continue;`.

Faithful throughout — conditions, arithmetic and control flow. No findings, no deviations.

## `Target_Is_Visible` ([Combat.c:2958](../../MoM/src/Combat.c#L2958), asm 121)

**The frame is exact, including the width of the third slot.** `distance` at `bp-8`, `player_idx` at
`bp-6`, and `enchantments` as a **`dword ptr`** at `bp-4`. Production declares them in that order with
`enchantments` as `uint32_t`, then `is_visible` (DI) and `other_battle_unit_idx` (SI) as registers.
`bu_ptr` and `u_ptr` are reconstruction aids with no slot.

**The enchantment OR combines exactly two sources**, and the in-code `OGBUG` note at
[Combat.c:2980](../../MoM/src/Combat.c#L2980) is correct. asm:42 and asm:44 OR the low and high words
of `battle_units[].enchantments` with `_UNITS[].enchantments` and nothing else — no
`item_enchantments`. Compare `Ranged_Mouse_Image`, which ORs four sources including items. Invisibility
granted by an item is genuinely invisible to this test in the original.

**The `UE_INVISIBILITY` test is on the low word** — `and dx, UE_INVISIBILITY / and ax, 0`
(asm:50-51), the usual split-32-bit form.

**The invisibility guard transcribes both jumps, not just one.** asm:53 `jnz loc_783E8` enters the
invisible-handling block on the enchantment bit; asm:61 `jz @@Done` leaves the function when the
`UA_INVISIBILITY` ability bit is also clear — and `@@Done` is the shared exit (`mov ax, is_visible /
pop / retf`), so that jump really is a return. Production writes the De Morgan inverse with an early
`return ST_TRUE` at [Combat.c:2986-2993](../../MoM/src/Combat.c#L2986-L2993), which maps one-to-one
onto those two jumps: the `&&` pair is the `jz`-to-exit path and the fall-through is the `jnz`-to-body
path. It returns the literal `ST_TRUE` where the listing returns `is_visible`, which asm:47 has just
set to `ST_TRUE` — same value.

**The two True Sight tests are independent `if`s, not a chain.** asm:66 `jnz` skips to `loc_783FD`,
and asm:69 sets the flag then falls into the same label — so both tests always run. Production writes
two separate `if` statements, which is right.

**The proximity loop's guards are faithful**: `status == bus_Active` (asm:88), `controller_idx !=
player_idx` (asm:97-98, `jz` skips), `Range_To_Battle_Unit(battle_unit_idx, other_battle_unit_idx)`
read off the push order, and `cmp [bp+distance], 2 / jge` giving `distance < 2`. The loop **runs to
completion** — asm:107 sets the flag and falls straight through to the increment at `loc_7845A`, with
no early exit anywhere in asm:105-112 — and production matches, setting `is_visible` and continuing.

### One trivial thing

[Combat.c:2995](../../MoM/src/Combat.c#L2995) ends `is_visible = ST_FALSE;;` — a stray second
semicolon.
