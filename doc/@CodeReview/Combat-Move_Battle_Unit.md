...
| --- | --- | --- |
| `Reload_Battle_Unit_Move_Sound` | [Combat.c:15794](../../MoM/src/Combat.c#L15794) | `ovr124/Reload_Battle_Unit_Move_Sound.asm` | 63 |
| `Move_Battle_Unit` | [Combat.c:2550](../../MoM/src/Combat.c#L2550) | `ovr091/Move_Battle_Unit.asm` |
| `Get_Battle_Unit_Move_Sound_Buffer` | [Combat.c:5057](../../MoM/src/Combat.c#L5057) | `ovr098/Get_Battle_Unit_Move_Sound_Buffer.asm` |
| `Check_Wall_Of_Fire_Attack` | [Combat.c:15878](../../MoM/src/Combat.c#L15878) | `ovr124/Check_Wall_Of_Fire_Attack.asm` |
| `Battle_Unit_Teleport` | [Spells133.c:706](../../MoM/src/Spells133.c#L706) | `ovr133/BU_Teleport.asm` |
| `Battle_Unit_Tunnel` | [Spells133.c:796](../../MoM/src/Spells133.c#L796) | `ovr133/BU_TunnelTo.asm` |
| `Battle_Unit_Compose_Bitmap` | [Combat.c:6283](../../MoM/src/Combat.c#L6283) | `ovr103/BU_CreateImage__SEGRAX.asm` |
| `Battle_Unit_Set_Animation_Flags` | [Combat.c:7365](../../MoM/src/Combat.c#L7365) | `ovr105/BU_SetBaseAnims__WIP.asm` |
| `Battle_Unit_Is_Airborne` | [Combat.c:4964](../../MoM/src/Combat.c#L4964) | `ovr098/BU_CheckFlight__WIP.asm` |
| `Battle_Unit_Set_Invisibility_Effect` | [Combat.c:7260](../../MoM/src/Combat.c#L7260) | `ovr105/BU_SetVisibility__WIP.asm` |
...
# `Reload_Battle_Unit_Move_Sound`

`o124p07`, walked 2026-08-27, 63 asm lines. Faithful, no findings.

## `Reload_Battle_Unit_Move_Sound` ([Combat.c:15794](../../MoM/src/Combat.c#L15794), asm 63)

Loads the movement sound for a unit from its unit-type `Sound` field.

No stack locals; `sound_num` is SI. asm:23-28 reads `_unit_type_table[type].Sound` as a zero-extended
byte (`mov ah, 0`) and divides by 16 — production writes `>> 4`, which is identical for a value that
cannot be negative.

Three arms, in listing order: `sound_num == 0` loads `SFX_Silence` from SOUNDFX (asm:29-38),
`sound_num == 13` loads entry 0 from NEWSOUND (asm:41-46), and anything else loads `sound_num` from
SOUNDFX (asm:48-51). The listing cross-jumps the last two into a shared `push ax / call
LBX_Reload_Next / add sp, 6` tail at `loc_A7E70`; production writes each call out in full, which is
correct.

The `*sound_seg_size = lbxload_entry_length;` on all three paths has no asm counterpart — the OG
reads that global at the call site instead. It is a port-side out-parameter and is marked
`/* HACK */` in the signature.
