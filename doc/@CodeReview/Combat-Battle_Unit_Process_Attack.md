...
| --- | --- | --- | --- |
| `Eliminated_Opponent` | [Combat.c:12010](../../MoM/src/Combat.c#L12010) | `ovr113/Eliminated_Opponent.asm` | 89 |
| `Battle_Unit_Process_Attack` | [Combat.c:12531](../../MoM/src/Combat.c#L12531) | `ovr122/BU_ProcessAttack__WIP.asm` (1236 lines) | faithful (R1-R9 fixed) |
| `Battle_Unit_Attack_Immunities` | [Combat.c:12217](../../MoM/src/Combat.c#L12217) (`static`) | `ovr122/Battle_Unit_Attack_Immunities.asm` | faithful |
| `Battle_Unit_Attack_Magic_Realm` | [Combat.c:13157](../../MoM/src/Combat.c#L13157) | `ovr122/Battle_Unit_Attack_Magic_Realm.asm` + `_misc.asm` | faithful (R11 fixed) |
| `Battle_Unit_Has_Ranged_Attack` | [Combat.c:13878](../../MoM/src/Combat.c#L13878) | `ovr122/Battle_Unit_Has_Ranged_Attack.asm` | faithful (R10 fixed) |
| `Range_To_Battle_Unit` | [Combat.c:13656](../../MoM/src/Combat.c#L13656) | `ovr122/Range_To_Battle_Unit.asm` | faithful |
| `Combat_Resistance_Check` | [Combat.c:12101](../../MoM/src/Combat.c#L12101) | `ovr122/Combat_Resistance_Check.asm` | faithful |
| `Combat_Effective_Resistance` | [Combat.c:12141](../../MoM/src/Combat.c#L12141) | `ovr122/Combat_Effective_Resistance.asm` | faithful (R12-R15 fixed) |
| `Battle_Unit_Defense_Special` | [Combat.c:12991](../../MoM/src/Combat.c#L12991) | `ovr122/Battle_Unit_Defense_Special.asm` | faithful (R16, R17 fixed) |
| `Combat_Roll_Damage_Dealt` | [Combat.c:12060](../../MoM/src/Combat.c#L12060) | `ovr122/CMB_AttackRoll__SEGRAX.asm` | faithful |
| `Combat_Roll_Damage_Blocked` | [Combat.c:12081](../../MoM/src/Combat.c#L12081) | `ovr122/CMB_DefenseRoll__SEGRAX.asm` | faithful |
| `Battle_Unit_Heal` | [Combat.c:8327](../../MoM/src/Combat.c#L8327) | `ovr110/Battle_Unit_Heal.asm` | faithful (R18, R19 fixed) |
| `Compute_Battle_Unit_Damage_From_Spell` | [Combat.c:11182](../../MoM/src/Combat.c#L11182) | `ovr113/Apply_Battle_Unit_Damage_From_Spell.asm` | faithful (R21-R24 fixed) |
| `Battle_Unit_Commit_Damage` | [Combat.c:11347](../../MoM/src/Combat.c#L11347) | `ovr113/Battle_Unit_Commit_Damage.asm` | faithful (R20 fixed) |
...
`o113p16`, walked 2026-08-27, 89 asm lines. Faithful, no findings.

## `Eliminated_Opponent` ([Combat.c:12010](../../MoM/src/Combat.c#L12010), asm 89)

Counts live units per side and names a winner when one side reaches zero.

No stack locals — `itr` is CX, `attacker_count` SI, `defender_count` DI; production declares all
three. The three zeroing stores at asm:10-12 are all present.

**The confusion swap is symmetric and both halves are the right way round.** For an
attacker-controlled unit, `Confusion_State == 2` increments `defender_count` and anything else
increments `attacker_count` (asm:37-42); the defender-side twin at asm:60-65 is the mirror.
Production writes the test as `!= 2` first, matching the `jnz` polarity of the listing rather than
inverting it.

**The defender test really is an `else`.** asm:44 `jmp short loc_92153` leaves the attacker branch
past the defender test, so the nested `else { if (...) }` production writes is the same shape.

Exits are `or si, si / jnz` for `attacker_count == 0` returning the defender, then `defender_count ==
0` returning the attacker, then `ST_UNDEFINED` (asm:71-82).

### Worth knowing — this duplicates part of `Check_For_Winner`

The counting loop here is the same logic as the head of
[Check_For_Winner](../../MoM/src/Combat.c#L6881), minus the turn-limit rule and the AI flee path.
Two functions in different overlays doing the same count. Note the labelling is inconsistent between
them: `Check_For_Winner` carries `OGBUG` comments on its `confusion_state == 2` tests, and this one
carries none, though the code is identical. Whatever is decided about those labels should be applied
to both.
