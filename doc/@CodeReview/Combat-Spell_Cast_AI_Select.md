...
**Status: DONE-DONE 2026-08-27.** Eight functions, 4,714 asm lines - all of `ovr139` plus the two `ovr110` effective-strength estimators, every body walked end to end. Four findings were raised and all four are fixed. All six were homeless before this doc.

## Scope

| function | production | listing | asm | state |
| --- | --- | --- | --- | --- |
| `Get_Effective_Melee_Strength` | [Combat.c:8584](../../MoM/src/Combat.c#L8584) | `ovr110/Get_Effective_Melee_Strength.asm` | 205 | **full body walk — faithful** |
| `Get_Effective_Ranged_Strength` | [Combat.c:8452](../../MoM/src/Combat.c#L8452) | `ovr110/Get_Effective_Ranged_Strength.asm` | 143 | **full body walk — faithful** |
| `AI_Score_Combat_Spell` | [Combat.c:16760](../../MoM/src/Combat.c#L16760) | `ovr139/AI_EvaluateCmbtSpell.asm` | 3,093 | **full body walk — faithful** |
| `AI_Build_Unit_Realm_Threat_Percentages` | [Combat.c:18006](../../MoM/src/Combat.c#L18006) | `ovr139/AI_UnitThreatRealms.asm` | 527 | **full body walk — faithful** |
| `AI_Build_Castable_Combat_Spell_List` | [Combat.c:18092](../../MoM/src/Combat.c#L18092) | `ovr139/AI_CombatSpellList.asm` | 287 | **full body walk — faithful** |
| `AI_Prepare_Combat_Realm_Threats` | [Combat.c:17948](../../MoM/src/Combat.c#L17948) | `ovr139/AI_SetCombatRealms.asm` | 171 | **full body walk — faithful** |
| `Effective_Battle_Unit_Strength` | [Combat.c:17923](../../MoM/src/Combat.c#L17923) | `ovr139/Effective_Battle_Unit_Strength.asm` | 165 | **full body walk — faithful** |
| `AI_Select_Combat_Spell` | [Combat.c:16711](../../MoM/src/Combat.c#L16711) | `ovr139/AI_SelectCmbtSpell.asm` | 123 | **full body walk — faithful** |
...
### `AI_Build_Unit_Realm_Threat_Percentages` ([Combat.c:18006](../../MoM/src/Combat.c#L18006), asm 527)
...
### `AI_Build_Castable_Combat_Spell_List` ([Combat.c:18092](../../MoM/src/Combat.c#L18092), asm 287)
...
### `AI_Score_Combat_Spell` ([Combat.c:16760](../../MoM/src/Combat.c#L16760), asm 3,093)
...
### `AI_Select_Combat_Spell` ([Combat.c:16711](../../MoM/src/Combat.c#L16711), asm 123)
...
### `AI_Prepare_Combat_Realm_Threats` ([Combat.c:17948](../../MoM/src/Combat.c#L17948), asm 171)
...
### `Effective_Battle_Unit_Strength` ([Combat.c:17923](../../MoM/src/Combat.c#L17923), asm 165)
...
# The two effective-strength estimators

`o110p05` and `o110p06`, walked 2026-08-27, 348 asm lines. They are the last two `ovr110` slots and
the siblings of `Get_Effective_Hits`, adjudicated in
[Combat-Miscellaneous.md](Combat-Miscellaneous.md). Both are AI threat valuations summed by
`Strategic_Combat` ([Combat.c:7882](../../MoM/src/Combat.c#L7882),
[Combat.c:7895](../../MoM/src/Combat.c#L7895)) and by `Effective_Battle_Unit_Strength`
([Combat.c:18064](../../MoM/src/Combat.c#L18064)).

Structurally faithful throughout. One operand-order deviation.

## The attack-flags guard is a signed `> 0`, and that matters

Both functions gate their entire attack-flag bonus block the same way — `Get_Effective_Ranged_Strength`
asm:63-65 and `Get_Effective_Melee_Strength` asm:107-109:

```
or      _SI_attack_flags, _SI_attack_flags
jg      short loc_89FBF          ; enter the bonus block
jmp     @@JmpJmpDone_Return_CX   ; otherwise return the base threat
```

`or reg, reg` clears OF, so `jg` is true only when the value is **positive as a signed 16-bit
integer** — the source condition is `attack_flags > 0`, not `!= 0`. Production writes `> 0` at
[Combat.c:8483](../../MoM/src/Combat.c#L8483) and
[Combat.c:8651](../../MoM/src/Combat.c#L8651).

**The distinction is observable, because the high bit is a real flag.**
[MOM_DEF.h:852](../../MoX/src/MOM_DEF.h#L852) defines `Att_WarpLghtn` as `0x8000`, and
`s_BATTLE_UNIT.Attack_Flags` is `int16_t` ([Combat.h:1345](../../MoM/src/Combat.h#L1345)). Any unit
whose combined attack attributes include Warp Lightning makes the parameter negative, so the OG skips
every flag bonus for it — Doom, Armor Piercing, Poison, Life Steal, Destruction, Stoning Touch, Death
Touch, Illusion, First Strike and Eldritch Weapon — and returns the base threat. The AI therefore
*under*-values exactly the units carrying the game's nastiest ranged attack. The callers pass
`(attack_attributes | ranged_attack_attributes)`, so any other flag OR'd with `Att_WarpLghtn` still
lands negative.

Anyone tempted to "simplify" either guard to `!= 0` would silently change AI threat valuation across
the whole game. Leave both as `> 0`.

## Deviation — operand order in the `> 3` products

Three sites compute a product in the opposite order to the listing:

| production | listing |
| --- | --- |
| `((figures * 20) * (ranged + -2))` ([8485](../../MoM/src/Combat.c#L8485)) | `(ranged - 2)` then `imul figures` then `imul 20` (ranged asm:25-29) |
| `((figures * 20) * (melee + -2))` ([8618](../../MoM/src/Combat.c#L8618)) | melee asm:25-29 |
| `((figures * 30) * (thrown + -2))` ([8649](../../MoM/src/Combat.c#L8649)) | thrown asm:68-72 |

The listing multiplies `(x - 2)` by `figures` first and scales last; production scales first. The
16-bit result is identical either way, so this is structure only — the same class as D3 elsewhere.
The `add ax, -2` spelling is why production writes `+ -2` rather than `- 2`, and that part is
faithful.

## `Get_Effective_Ranged_Strength` ([Combat.c:8452](../../MoM/src/Combat.c#L8452), asm 143)

No stack locals; `figures` (DI), `attack_flags` (SI) and `ranged_threat` (CX) are all registers, and
production declares the one local with its `// _CX_` annotation.

`if (ranged == 0) return 0` (asm:16-21), then the `> 3` arm, then a base of `figures * 40` refined by
a **compare chain** — `cmp ax, 1 / jz`, `cmp ax, 2 / jz`, `cmp ax, 3 / jz`, fall through (asm:37-44).
Three cases is below Borland's jump-table threshold, so production's `if` / `else if` chain is the
faithful shape. The divisors are 7, 3, and 3-after-doubling.

**Cross-jump tails.** `loc_89FA1` (`ax = cx`) is shared by cases 1 and 2, and `loc_89FA3`
(`cwd / idiv bx`) by all three; case 3 does its own `shl ax, 1` and jumps into the shared divide.
Production writes each case out in full, which is correct.

The bonus block is `Doom` **else** `ArmorPrc` (asm:67-82 — the Doom arm jumps past the ArmorPrc test),
then five independent `+= figures * 60` for Poison, LifeSteal, Destruct, StnTouch and DthTouch, then
Illusion `* 5` and EldrWeap `* 5 / 4`. All ten in listing order.

## `Get_Effective_Melee_Strength` ([Combat.c:8584](../../MoM/src/Combat.c#L8584), asm 205)

Two register locals, `melee_threat` (CX) and `thrown_threat` (DI), both annotated.

**Here the selectors really are jump tables, and production's `switch` is right.** `off_8A1ED` for
melee (asm:41) and `off_8A1E5` for thrown (asm:84), four entries each covering cases 0-3, bounded by
`cmp bx, 3 / ja`. Contrast the ranged function's compare chain above — the two make a clean
side-by-side of when Borland picks each.

**Both tables carry a dead `case 0`.** `loc_8A09E` and `loc_8A0F5` set the threat to zero, but melee
0 already returned at asm:16-21 and thrown 0 is excluded by `cmp [bp+thrown], 0 / jle` at asm:64.
Production keeps both cases. They are in the OG's tables, so they belong in the reconstruction.

The thrown block mirrors the melee one with different constants — `figures * 60` as the base and
`figures * 30` in the `> 3` arm — and `melee_threat += thrown_threat` (asm:106) closes it, carrying
production's `OGBUG` note that the `+=` should have sat inside the `thrown > 0` test.

The melee bonus block adds `Att_1stStrike` (asm:171-179) which the ranged one lacks — production has
the ranged omission marked `DNE` in place, correctly.

**The gaze tail is faithful.** asm:191-196: `ranged_type >= srat_StoneGaze` adds 300, and
`== srat_MultiGaze` adds a further 900 nested inside it. Production matches, including the nesting —
a Multi Gaze unit gets both, not just the 900.
