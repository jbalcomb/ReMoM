...
- **Outputs:** `End_Of_Combat()` (Combat.c:21256) writes unit death/status/XP, city losses (`combat_results_population_lost`, `combat_results_buildings_lost`), gold (`combat_results_gold`), lair guard updates; `Combat__WIP` then applies move-in, ownership change, relations.
