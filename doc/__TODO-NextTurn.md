...
- [x] **7F** [`Players_Check_Spell_Research`](../MoM/src/NEXTTURN.c#L3631) — **done-done**, doc [NextTurn/NEXTTURN-Players_Check_Spell_Research.md](NextTurn/NEXTTURN-Players_Check_Spell_Research.md). Per-turn, per-player: if research finished (`research_cost_remaining == 0` && live target) grant via `Player_Gets_Spell`; if a new target is needed (negated marker, or `spl_NONE` on turns <2) run the human picker (`Spell_Research_Select`) / `AI_Spell_Research_Select`, then seed the next `research_cost_remaining`. Verified 1:1 against `ovr140/Players_Check_Spell_Research.asm`. `IDK`→`did_learn_spell_anim`. One ReMoM deviation: the `DBG_trigger_complete_research_spell` debug hook (not in asm).
  - [x] **7F.1** [`Player_Gets_Spell`](../MoM/src/NEXTTURN.c#L3778) (o140p21, callee) — **done-done**, doc [NextTurn/NEXTTURN-Player_Gets_Spell.md](NextTurn/NEXTTURN-Player_Gets_Spell.md). Records a learned/traded/gifted/treasure/conquest spell: mark `spells_list`, discount SoM cost, and if it was the research target clear the candidate list + re-select. Verified 1:1 against `ovr140/Player_Gets_Spell.asm`. This session fixed R1 (`som_research_cost` formula — missing `/2`, assign→floor-subtract), R2 (else-loop compared `researching_spell_idx`→`spell_idx`), R3 (remaining-cost `spell_data_table[spell_idx]`→`[researching_spell_idx]`), R4 (`spl_UNDEFINED`→`spl_NONE`); builds clean.
