...
- Constants/fields: `events_table->*` (per-event `*_Status`/`*_Duration`/`*_Data`/`*_Player`), `et_Meteor … et_Mana_Short`, `es_Starting=1`/`es_Ongoing=2`/`es_Nothing=0`, `emt_Happened=3`/`emt_Ended=1`, `NUM_OVERLAND_ENCHANTMENTS = 24`, `NEUTRAL_PLAYER_IDX = 5`, `HUMAN_PLAYER_IDX = 0`, `rt_Arcane`, `bt_TradeGoods`, `_current_player_idx`.
