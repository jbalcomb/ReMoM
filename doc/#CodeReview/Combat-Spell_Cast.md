...
**All three `Combat_Cast_Spell` call sites.** Each pushes `_combat_wp`, `_combat_wy`, `_combat_wx`, then the caster, and cleans `add sp, 8`. The player case builds its caster as `mov ax, [_human_player_idx]` / `add ax, 20` -- a raw `20`, which is what [Combat.c:7223](../../MoM/src/Combat.c#L7223) writes rather than `CASTER_IDX_BASE`, and the literal is the faithful reading. The unit case and the no-selection case both pass `caster_id` straight through.
