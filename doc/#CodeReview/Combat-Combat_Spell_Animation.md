...
| `Wall_Rise_Load` | [Combat.c:22998](../../MoM/src/Combat.c#L22998) | `ovr163/Wall_Rise_Load.asm` | 109 | **full body walk — faithful** |
...
| `Combat_Load_Spell_Sound_Effect` | [Combat.c:16454](../../MoM/src/Combat.c#L16454) | `ovr124/Combat_Load_Spell_Sound_Effect.asm` | 64 | **full body walk — faithful** |
...
### `Wall_Rise` ([Spells133.c:319](../../MoM/src/Spells133.c#L319), asm 101) and `Wall_Rise_Load` ([Combat.c:22998](../../MoM/src/Combat.c#L22998), asm 109)
...
**It reallocates all eighteen battle-figure blocks first.** asm:17-30 is `Allocate_Next_Block(_screen_seg, 55)` into `battle_unit_picts_seg[itr]` for `itr < 18`, before any wall graphics load - [Combat.c:23007](../../MoM/src/Combat.c#L23007). The preserved `WARNING: redoing the pointers may cause trouble` sits on the right line.

**The three load cases differ in both count and base offset.** Type 0 loads **12** entries based at `_combat_wall_sprite_bank * 12` - IDA's `CMB_StoneWallType` - while types 1 and 2 load **14** each, based at `36` and `50`. asm:67 is `add ax, 36` and asm:87 is `add ax, 50`, matching [Combat.c:23025](../../MoM/src/Combat.c#L23025) and [Combat.c:23033](../../MoM/src/Combat.c#L23033) where production spells them `(3 * 12)` and `((3 * 12) + 14)`. The asymmetry is real: only the stone-wall set is bank-selected.

**`Release_Block(_screen_seg)`** closes the `Mark_Block` at the top - [Combat.c:23038](../../MoM/src/Combat.c#L23038).

### `Cast_Call_Chaos` ([Spells133.c:1452](../../MoM/src/Spells133.c#L1452), asm 472) and `Apply_Call_Chaos` ([Spells133.c:1607](../../MoM/src/Spells133.c#L1607), asm 180)
...
### `Combat_Load_Spell_Sound_Effect` ([Combat.c:16454](../../MoM/src/Combat.c#L16454), asm 64)
