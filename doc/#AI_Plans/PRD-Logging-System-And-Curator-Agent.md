...
- **Testing the Win32 SEH handler requires `--gtest_catch_exceptions=0` (or `::testing::GTEST_FLAG(catch_exceptions) = false`).** gtest wraps every test body — including death-test bodies — in a stack-based `__try/__except` block that catches SEH *before* `SetUnhandledExceptionFilter`-registered filters can run. With gtest's catch enabled, an access violation in a death test is intercepted by gtest, gtest's `__except` exits the child process via the C runtime's normal exit path (which runs `atexit` handlers), and `log_atexit_handler` ends up draining the ring — so the test sees the pre-crash message but *not* the `[CRASH]` marker, because the SEH filter never ran. Disabling catch_exceptions for just the crash test (scoped via the gtest flag, restored after the assertion) lets the OS dispatch the access violation to STU_LOG's filter as intended. The flag value at death-test spawn time is what the child inherits, so an in-body toggle around the `ASSERT_DEATH` is sufficient.
