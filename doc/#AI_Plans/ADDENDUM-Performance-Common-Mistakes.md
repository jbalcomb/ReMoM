...
| 12 | **Single run / no warmup / benchmark noise** | One sample lies; first run pays cold-cache costs | 🔵 Planned | Percentiles over many frames + deterministic re-runs; Mode A should discard a warmup turn and report a stable band, not one number |
...
| 15 | **No regression guard — a fix silently rots** | The next change quietly undoes the win | 🔵 Planned | CTest gate asserts percentiles vs. committed baseline, fails the build on regression (Mode A + Mode B) |
