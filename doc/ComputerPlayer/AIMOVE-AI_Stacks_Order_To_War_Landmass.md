...
**Location:** [MoM/src/AIMOVE.c:1899](../../MoM/src/AIMOVE.c#L1899) (~64 lines, ends 1962).
...
Runs as dispatch slot 10 of [`AI_Set_Unit_Orders`](AIMOVE-AI_Set_Unit_Orders.md), immediately after slot 9 [`AI_Stacks_Roamers_Target_Or_Deploy`](AIMOVE-AI_Stacks_Roamers_Target_Or_Deploy.md). The slot-9 function sets `lmt_Abandon` on landmasses that the AI wants to evacuate toward the war target — and the comment at [MOM_DAT.h:167](../../MoX/src/MOM_DAT.h#L167) explicitly notes that `lmt_Abandon` "seems made specifically for AI_Stacks_Order_To_War_Landmass, which runs next."
...
```c
if(
    (cp_landmass_type_array[landmass_idx] >= lmt_Abandon)        // lmt_Abandon, lmt_NoTargets
...
- Sets `g_ai_set_target_caller = 8` at [line 1958](../../MoM/src/AIMOVE.c#L1958) for the debug-instrumentation trail.
...
### Phase 1 — Early-out if no war target ([lines 1912-1916](../../MoM/src/AIMOVE.c#L1912-L1916))
...
### Phase 2 — Per-stack filter ([lines 1917-1935](../../MoM/src/AIMOVE.c#L1917-L1935))
...
The combined effect: this function pulls mobile units from landmasses other than the war target. The dispatcher's gate ([line 187-199](../../MoM/src/AIMOVE.c#L187-L199)) typically calls this function for `lmt_Abandon`, `lmt_NoTargets`, `lmt_Own`, `lmt_NoOwnCityAndAllyHasCity`, and `lmt_NoOwnCity` landmasses — the `lmt_Abandon` case being the explicit "evacuate to war target" path per the [MOM_DAT.h:158-170](../../MoX/src/MOM_DAT.h#L158-L170) annotation.

### Phase 3 — Per-unit filter and order ([lines 1936-1959](../../MoM/src/AIMOVE.c#L1936-L1959))
...
**Suspected OGBUG at [line 1939](../../MoM/src/AIMOVE.c#L1939)** — marked in source with `/* ¿ OGBUG  should check wind-walk and items ? */`. The mobility check uses `Unit_Has_AirTravel` / `Unit_Has_WaterTravel` which presumably look at the unit type's base abilities and movement-type byte. But a unit could gain air/water travel from the **Wind Walking** spell (UA_WINDWALKING granted to other stack-mates) or from an **enchanted item** (Flight, Water Walking) — neither of which would necessarily be reflected in those helpers. A unit so-enhanced would be skipped here even though it could cross water for the purposes of joining the war landmass. Whether this is fixable depends on what the helpers actually check; not actionable until the helpers are walked through.
...
| 3 | `AI_Survey_Excess_Units` | Score surplus combat units into `G_Pushout_*` / `G_Seafaring_*` |
...
| 9 | [`AI_Stacks_Roamers_Target_Or_Deploy`](AIMOVE-AI_Stacks_Roamers_Target_Or_Deploy.md) | Assign targets to roamer stacks; sets `lmt_Abandon` as needed |
| **10** | **`AI_Stacks_Order_To_War_Landmass`** | **(this function)** — pull mobile non-melder units to war landmass stage point |
| 11 | `G_AI_HomeRallyFill__WIP` | Home rally fill |
| 13 | `G_AI_RallyFill__WIP` | Continent rally fill from pushout pool |
| 14 | `AI_FillGarrisons__WIP` | Top up garrisons |

The slot-9 → slot-10 sequencing matters: slot 9's `AI_Stacks_Roamers_Target_Or_Deploy` is the only function that sets `lmt_Abandon`, and the slot-10 gate accepts landmasses tagged `lmt_Abandon`. So an `lmt_Abandon` write in slot 9 directly feeds slot 10's `AI_Stacks_Order_To_War_Landmass` on the same (plane, landmass) iteration.
...
- [AIMOVE-AI_Stacks_Roamers_Target_Or_Deploy.md](AIMOVE-AI_Stacks_Roamers_Target_Or_Deploy.md) — sole writer of `lmt_Abandon` (the tag that drives most calls into this function)
...
- [MoX/src/MOM_DAT.h:158-170](../../MoX/src/MOM_DAT.h#L158-L170) — `lmt_Abandon` setting conditions, including the "made specifically for AI_Stacks_Order_To_War_Landmass" annotation
