...
- **Direct damage (`loc_8A236`, [11676](../../MoM/src/Combat.c#L11676)):** immunity-magic → Star Fires (Death/Chaos only) → Psionic Blast (illusion-immune skip) → Life Drain (Death/death-immune skip) → Righteousness-vs-Chaos/Death → controller==player → status → visibility → damage (`Undeath_Dmg = 13 - Combat_Effective_Resistance(*bu, sbr_Death)` for Life Drain, else `Apply_Battle_Unit_Damage_From_Spell(spell_idx, battle_unit_idx, &Reg_Dmg, 25)`) → `Target_Value = Reg+Undeath+Irrev`, bonus `(100 - figure_cnt*hits) + front_figure_damage`.
