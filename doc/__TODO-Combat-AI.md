...
- **Now regression-testable:** the AI combat turn is exercised end-to-end by the combat characterization tests (`HeMoM --combat` strategic, plus tactical scenarios via `ReMoM --scenario`); see [__TODO-Combat.md § Test coverage](__TODO-Combat.md). Reconstructing/finalizing the functions above can be validated against the pinned baselines.
