...
#endif
        LOG_INFO(LOG_CAT_SDL3_AUDIO, "ERROR:  Mix_Init():  %s", SDL_GetError());
    }
    else
    {
        LOG_INFO(LOG_CAT_SDL3_AUDIO, "SUCCESS:  Mix_Init():  %s", SDL_GetError());
    }
...
#endif
        LOG_INFO(LOG_CAT_SDL3_AUDIO, "ERROR:  Mix_OpenAudio():  %s", SDL_GetError());
    }
    else
    {
        LOG_INFO(LOG_CAT_SDL3_AUDIO, "SUCCESS:  Mix_OpenAudio():  %s", SDL_GetError());
    }

    // Set the volume of the music to 50% (half the MAX_VOLUME)
    LOG_INFO(LOG_CAT_SDL3_AUDIO, "Mix_VolumeMusic():  %d", Mix_VolumeMusic(-1));
...
wav_sound_chunk3 = Mix_LoadWAV_IO(SDL_IOFromMem(wav_sound_buffer3, wav_sound_buffer_size3), 0);
...
// #endif
    ...
    switch(lbx_sound_type)
    {
        ...
        case 2:  /* VOC file */
        {
            ...
            {
#ifdef STU_DEBUG
                LOG_DEBUG(LOG_CAT_PFL, "ERROR:  SDL_IOFromMem()  %s", SDL_GetError());
#endif
            }
            ...
            {
#ifdef STU_DEBUG
                LOG_DEBUG(LOG_CAT_PFL, "ERROR:  Mix_LoadWAV_IO()  %s", SDL_GetError());
#endif
            }
...
out_wav_buf
    , for Mix_LoadWAV_IO(SDL_IOFromMem())
out_wav_len
    , for Mix_LoadWAV_IO(SDL_IOFromMem())
...
//         // m->music = Mix_LoadMUSType_IO(rw, m->sdlmtype, 0);
//         m->music = Mix_LoadMUSType_IO(rw, MUS_MID, 0);  // NOTE:  need to call Mix_FreeMusic(m->music) later (probably, call  Mix_HaltMusic() first)
//         // if(m->music == NULL) { ... }
//         SDL_CloseIO(rw);
//     }
// 
...
//     if(!m->music)
//     {
//         // log_error("SDLA: Mix_LoadMUSType_IO failed on music %i (type %i): %s\n", mus_index, m->type, SDL_GetError());
