...
Manhattan Distance      AKA "taxicab distance" or "L1 distance"
|x1 - x2| + |y1 - y2|
more often use in 2D games, as it doesn't suffer from the issues that Euclidean distance does
notable difference Manhattan distance treats diagonal moves as being twice as expensive as a move in a cardinal direction
...
*/
#define DIST(_x1_,_x2_,_y1_,_y2_)               ( abs( ( (_x1_) - (_x2_) ) ) + abs( ( (_y1_) - (_y2_) ) ) )
#define ABS_DIST(_x1_,_x2_,_y1_,_y2_)           ( abs( ( (_x1_) - (_x2_) ) ) + abs( ( (_y1_) - (_y2_) ) ) )
#define MANHATTAN_DISTANCE(_x1_,_x2_,_y1_,_y2_) ( abs( ( (_x1_) - (_x2_) ) ) + abs( ( (_y1_) - (_y2_) ) ) )
...
/*
    Player / Wizard
*/
#define PLAYER_NUM(_player_idx_)      ( (_player_idx_) + 1 )
#define PLAYER_IDX(_player_num_)      ( (_player_num_) - 1 )
...
*/
/* DELETEME */  #define GET_TERRAIN_TYPE(_wx_, _wy_, _wp_)  ( (int16_t)GET_2B_OFS(_world_maps, (((_wp_) * WORLD_SIZE * 2) + ((_wy_) * WORLD_WIDTH * 2) + ((_wx_) * 2))) )
...
...( & 0x0F) ~== 'any mineral'
*/
#define GET_TERRAIN_SPECIAL(_wx_, _wy_, _wp_)           ( GET_1B_OFS( _map_square_terrain_specials, ( ((_wp_) * WORLD_SIZE) + ((_wy_) * WORLD_WIDTH) + (_wx_) )            ) )
#define SET_TERRAIN_SPECIAL(_wx_, _wy_, _wp_, _value_)  ( SET_1B_OFS( _map_square_terrain_specials, ( ((_wp_) * WORLD_SIZE) + ((_wy_) * WORLD_WIDTH) + (_wx_) ), (_value_) ) )
...
#define ADD_MAP_SQUARE_FLAG(_wx_, _wy_, _wp_, _value_)  ( _map_square_flags[ ( ( (_wp_) * WORLD_SIZE ) + ( (_wy_) * WORLD_WIDTH ) + (_wx_) ) ] |= (_value_) )
#define REM_MAP_SQUARE_FLAG(_wx_, _wy_, _wp_, _value_)  ( _map_square_flags[ ( ( (_wp_) * WORLD_SIZE ) + ( (_wy_) * WORLD_WIDTH ) + (_wx_) ) ] ^= (_value_) )

#define CLEAR_MAP_SQUARE_FLAGS_VOLCANO_PLAYER_IDX()  ( SET_MAP_SQUARE_FLAG(wx, wy, wp, (GET_MAP_SQUARE_FLAG(wx, wy, wp) & 0xF8)) )  /* 0b11111000 */

#define MAP_SQUARE_HAS_NO_ROAD(_wx_, _wy_, _wp_)  ( GET_MAP_SQUARE_FLAG((_wx_), (_wy_), (_wp_)) & TF_NoRoad80 )
#define MAP_SQUARE_HAS_ROAD(_wx_, _wy_, _wp_)  ( GET_MAP_SQUARE_FLAG((_wx_), (_wy_), (_wp_)) & (MSF_ROAD | MSF_EROAD) )
#define MAP_SQUARE_HAS_ROAD_REGULAR(_wx_, _wy_, _wp_)  ( GET_MAP_SQUARE_FLAG((_wx_), (_wy_), (_wp_)) & MSF_ROAD )
#define MAP_SQUARE_HAS_ROAD_ENCHANTED(_wx_, _wy_, _wp_)  ( GET_MAP_SQUARE_FLAG((_wx_), (_wy_), (_wp_)) & MSF_EROAD )
...
/*
    assumes _main_map_grid_x, _main_map_grid_y
*/
...
// looks like both could have existed, maybe even MOX vs. MOM, with just one type in NEXTTURN.C
// just two different peoples ideas of a maximum integer value that leaves a little header to avoid an overflow?
#define MAX_SINT            30000
#define MAX_SINT2           32000
#define MAX_GOLD_RESERVE    30000
#define MAX_MANA_RESERVE    30000
...
#define CLS_ON() { Set_Page_On();   Fill(SCREEN_XMIN, SCREEN_YMIN, SCREEN_XMAX, SCREEN_YMAX, ST_TRANSPARENT);  Set_Page_Off(); }
#define CLROFF() { Set_Page_Off();  Fill(SCREEN_XMIN, SCREEN_YMIN, SCREEN_XMAX, SCREEN_YMAX, ST_TRANSPARENT);                  }
#define CLRON()  { Set_Page_On();   Fill(SCREEN_XMIN, SCREEN_YMIN, SCREEN_XMAX, SCREEN_YMAX, ST_TRANSPARENT);                  }
