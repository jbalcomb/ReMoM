...
// WZD dseg:8FAA
int16_t * _ai_reevaluate_continents_countdown;  /* 2-byte, signed */
// WZD dseg:8FAC
/*
landmass_idx
...
// WZD dseg:9296 00 00                                           RP_AI_UnsetRealmVar2 dw 0               ; DATA XREF: AI_Score_Combat_Spell+141r
...
*/
uint8_t * _map_square_terrain_specials;                 // load in Load_SAVE_GAM()
...
// WZD dseg:BFAA
int16_t city_screen_change_button;
...
byte_ptr IDK_CityScr_15x50_tmpbuf[15];

// WZD dseg:BFDC
// ; FOOD_BRKDN *Retn_Struct
byte_ptr Retn_Struct;
...
// WZD dseg:BFE2
/*
LBX_Reload_Next(help_lbx_file__ovr055, 1, _screen_seg);
*/
SAMB_ptr word_42A82;

// WZD dseg:BFE4
/*
LBX_Reload(help_lbx_file__ovr055, 0, _screen_seg);
*/
SAMB_ptr IMG_Seg;
...
// WZD dseg:C044
// MainScr  int16_t CRP_OVL_Obstacle_Var1;
...
Consumed in AI_Turn (AIDUDES.c:249): right after an AI player completes casting a spell, if the flag is true the AI re-runs Allocate_AI_Data, Player_Hostile_Opponents, and AI_Player_Calculate_Target_Values to refresh its hostility list and target-value scoring.
Purpose: avoid recomputing the AI's expensive world-evaluation tables on every turn — only refresh them when a spell that just resolved would have invalidated them (new units appeared, units died, treaties/diplomacy shifted, overland enchantments changed, etc.). It's effectively a dirty-bit on the AI's cached view of the world.
