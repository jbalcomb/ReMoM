...
void VGA_DAC_Init(char *PaletteLbxFileName)
{
    ...
    fp_WTF = SA_Allocate_Space(64);
    sa_WTF = FP_SEG(fp_WTF);
    gsa_Palette = sa_WTF;
#ifdef DEBUG
    dlvfprintf("DEBUG: [%s, %d] fp_WTF: %p\n", __FILE__, __LINE__, fp_WTF);
    dlvfprintf("DEBUG: [%s, %d] fp_WTF: %Fp\n", __FILE__, __LINE__, fp_WTF);
    dlvfprintf("DEBUG: [%s, %d] sa_WTF: 0x%04X\n", __FILE__, __LINE__, sa_WTF);
